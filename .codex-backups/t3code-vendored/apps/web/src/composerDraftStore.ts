...
const QueuedComposerEntrySchema = Schema.Struct({
  id: Schema.String,
  prompt: Schema.String,
  createdAt: Schema.String,
});
export type QueuedComposerEntry = typeof QueuedComposerEntrySchema.Type;
...
const PersistedComposerThreadDraftState = Schema.Struct({
  ...
  queuedPrompts: Schema.optionalKey(Schema.Array(QueuedComposerEntrySchema)),
...
export interface ComposerThreadDraftState {
  ...
  queuedPrompts: QueuedComposerEntry[];
...
interface ComposerDraftStoreState {
  ...
  enqueueQueuedPrompt: (threadId: ThreadId, entry: QueuedComposerEntry) => void;
  prependQueuedPrompt: (threadId: ThreadId, entry: QueuedComposerEntry) => void;
  removeQueuedPrompt: (threadId: ThreadId, entryId: string) => void;
  moveQueuedPromptToFront: (threadId: ThreadId, entryId: string) => void;
  shiftQueuedPrompt: (threadId: ThreadId) => QueuedComposerEntry | null;
  restoreQueuedPromptToFront: (threadId: ThreadId, entry: QueuedComposerEntry) => void;
...
const EMPTY_QUEUED_PROMPTS: QueuedComposerEntry[] = [];
const EMPTY_TERMINAL_CONTEXTS: TerminalContextDraft[] = [];
Object.freeze(EMPTY_IMAGES);
Object.freeze(EMPTY_IDS);
Object.freeze(EMPTY_PERSISTED_ATTACHMENTS);
Object.freeze(EMPTY_QUEUED_PROMPTS);
...
function normalizeQueuedComposerEntry(value: unknown): QueuedComposerEntry | null {
  if (!value || typeof value !== "object") {
    return null;
  }
  const candidate = value as Record<string, unknown>;
...
function normalizePersistedDraftsByThreadId(
  ...
  for (const [threadId, draftValue] of Object.entries(rawDraftMap as Record<string, unknown>)) {
    ...
    const attachments = Array.isArray(draftCandidate.attachments)
      ...
      : [];
    const queuedPrompts = Array.isArray(draftCandidate.queuedPrompts)
      ? draftCandidate.queuedPrompts.flatMap((entry) => {
          const normalized = normalizeQueuedComposerEntry(entry);
          return normalized ? [normalized] : [];
        })
      : [];
...
function partializeComposerDraftStoreState(
  ...
  for (const [threadId, draft] of Object.entries(state.draftsByThreadId)) {
    ...
    const persistedDraft: DeepMutable<PersistedComposerThreadDraftState> = {
      ...
      ...(draft.queuedPrompts.length > 0 ? { queuedPrompts: draft.queuedPrompts } : {}),
      ...(draft.terminalContexts.length > 0
        ? {
...
function toHydratedThreadDraft(
  ...
  return {
    ...
    queuedPrompts: [...(persistedDraft.queuedPrompts ?? [])],
    terminalContexts:
...
export const useComposerDraftStore = create<ComposerDraftStoreState>()(
  persist(
    (set, get) => ({
      ...
      enqueueQueuedPrompt: (threadId, entry) => {
        if (threadId.length === 0) {
          return;
        }
        const normalizedEntry = normalizeQueuedComposerEntry(entry);
        if (!normalizedEntry) {
          return;
        }
        set((state) => {
          ...
          return {
            draftsByThreadId: {
              ...
              [threadId]: {
                ...existing,
                queuedPrompts: [...existing.queuedPrompts, normalizedEntry],
              },
            },
          };
        });
      },
      prependQueuedPrompt: (threadId, entry) => {
        if (threadId.length === 0) {
          return;
        }
        const normalizedEntry = normalizeQueuedComposerEntry(entry);
        ...
        set((state) => {
          ...
          return {
            draftsByThreadId: {
              ...
              [threadId]: {
                ...existing,
                queuedPrompts: [normalizedEntry, ...existing.queuedPrompts],
              },
            },
          };
        });
      },
      removeQueuedPrompt: (threadId, entryId) => {
        if (threadId.length === 0 || entryId.length === 0) {
          return;
        }
        set((state) => {
          const current = state.draftsByThreadId[threadId];
          ...
          const nextDraft: ComposerThreadDraftState = {
            ...current,
            queuedPrompts: current.queuedPrompts.filter((entry) => entry.id !== entryId),
          };
          const nextDraftsByThreadId = { ...state.draftsByThreadId };
          if (shouldRemoveDraft(nextDraft)) {
            delete nextDraftsByThreadId[threadId];
      ...
      moveQueuedPromptToFront: (threadId, entryId) => {
        if (threadId.length === 0 || entryId.length === 0) {
          return;
        }
        set((state) => {
          const current = state.draftsByThreadId[threadId];
          if (!current || current.queuedPrompts.length < 2) {
            return state;
          }
          const targetIndex = current.queuedPrompts.findIndex((entry) => entry.id === entryId);
          if (targetIndex <= 0) {
            return state;
          }
          const targetEntry = current.queuedPrompts[targetIndex];
          if (!targetEntry) {
            return state;
          }
          return {
            draftsByThreadId: {
              ...
              [threadId]: {
                ...
                queuedPrompts: [
                  targetEntry,
                  ...current.queuedPrompts.filter((entry) => entry.id !== entryId),
                ],
              },
            },
          };
      ...
      shiftQueuedPrompt: (threadId) => {
        if (threadId.length === 0) {
          return null;
        }
        let shiftedEntry: QueuedComposerEntry | null = null;
        set((state) => {
          const current = state.draftsByThreadId[threadId];
          const nextEntry = current?.queuedPrompts[0] ?? null;
          if (!current || !nextEntry) {
            return state;
          }
          shiftedEntry = nextEntry;
          const nextDraft: ComposerThreadDraftState = {
            ...current,
      ...
      restoreQueuedPromptToFront: (threadId, entry) => {
        get().prependQueuedPrompt(threadId, entry);
      },
...
export function useQueuedComposerPrompts(threadId: ThreadId): ReadonlyArray<QueuedComposerEntry> {
  return useComposerDraftStore(
    (state) => state.draftsByThreadId[threadId]?.queuedPrompts ?? EMPTY_QUEUED_PROMPTS,
  );
}
...
export function clearPromotedDraftThreads(serverThreadIds: ReadonlySet<ThreadId>): void {
  useComposerDraftStore.setState((state) => {
    const promotedDraftThreadIds = (Object.keys(state.draftThreadsByThreadId) as ThreadId[]).filter(
      (draftId) => serverThreadIds.has(draftId),
    );
    if (promotedDraftThreadIds.length === 0) {
      return state;
    }
    const promotedThreadIdSet = new Set(promotedDraftThreadIds);
    const nextProjectDraftThreadIdByProjectId = Object.fromEntries(
      Object.entries(state.projectDraftThreadIdByProjectId).filter(
        ([, draftThreadId]) => !promotedThreadIdSet.has(draftThreadId),
      ),
    ) as Record<ProjectId, ThreadId>;
    const nextDraftThreadsByThreadId = { ...state.draftThreadsByThreadId };
    for (const draftId of promotedDraftThreadIds) {
