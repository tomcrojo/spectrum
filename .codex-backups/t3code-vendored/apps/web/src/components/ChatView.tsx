...
import { useEmbeddedFollowUpBehavior } from "../hooks/useEmbeddedFollowUpBehavior";
...
export type ChatViewUiMode = "full" | "embedded";

interface ChatViewProps {
  threadId: ThreadId;
  uiMode?: ChatViewUiMode;
}
...
export default function ChatView({ threadId, uiMode = "full" }: ChatViewProps) {
  ...
  const embeddedFollowUpBehavior = useEmbeddedFollowUpBehavior();
  const effectiveFollowUpBehavior =
    uiMode === "embedded" ? embeddedFollowUpBehavior ?? settings.followUpBehavior : settings.followUpBehavior;
  const setStickyComposerModelSelection = useComposerDraftStore(
    (store) => store.setStickyModelSelection,
  );
  const timestampFormat = settings.timestampFormat;
  const navigate = useNavigate();
  const threadRoutePath = uiMode === "embedded" ? "/embed/thread/$threadId" : "/$threadId";
  ...
  const queuedPrompts = useQueuedComposerPrompts(threadId);
  ...
  const enqueueComposerQueuedPrompt = useComposerDraftStore((store) => store.enqueueQueuedPrompt);
  const prependComposerQueuedPrompt = useComposerDraftStore((store) => store.prependQueuedPrompt);
  const removeComposerQueuedPrompt = useComposerDraftStore((store) => store.removeQueuedPrompt);
  const shiftComposerQueuedPrompt = useComposerDraftStore((store) => store.shiftQueuedPrompt);
  const restoreComposerQueuedPromptToFront = useComposerDraftStore(
    (store) => store.restoreQueuedPromptToFront,
  );
    ...
    async (input: { branch: string; worktreePath: string | null; envMode: DraftThreadEnvMode }) => {
      ...
      if (storedDraftThread) {
        ...
        if (storedDraftThread.threadId !== threadId) {
          if (uiMode === "embedded") {
            throw new Error("Embedded panels cannot switch to a different thread.");
          }
          await navigate({
            to: threadRoutePath,
      ...
      if (uiMode === "embedded") {
        throw new Error("Embedded panels cannot create or switch to a different thread.");
      }
      await navigate({
        to: threadRoutePath,
  ...
  }, [diffOpen, navigate, threadId, threadRoutePath]);
  ...
  const buildComposerSubmissionSnapshot = useCallback((): ComposerSubmissionSnapshot | null => {
  ...
  }, [composerImages, composerTerminalContexts]);

  const enqueueSubmission = useCallback(
    (snapshot: ComposerSubmissionSnapshot, placement: "append" | "prepend" = "append"): boolean => {
      if (!activeThread) {
        return false;
      }
      if (snapshot.images.length > 0 || composerTerminalContexts.length > 0) {
        toastManager.add({
          type: "warning",
          title: "Only text follow-ups can be queued",
          description: "Send images or terminal context immediately instead of queueing them.",
        });
        return false;
      }
      if (!snapshot.trimmedPrompt) {
        return false;
      }
      const entry: QueuedComposerEntry = {
        id: randomUUID(),
        prompt: snapshot.trimmedPrompt,
        createdAt: new Date().toISOString(),
      };
      if (placement === "prepend") {
        prependComposerQueuedPrompt(activeThread.id, entry);
      } else {
        enqueueComposerQueuedPrompt(activeThread.id, entry);
      }
      promptRef.current = "";
    ...
    [
      ...
      enqueueComposerQueuedPrompt,
      prependComposerQueuedPrompt,
      scheduleComposerFocus,
    ],
  );
  ...
  const dispatchSubmission = useCallback(
    async (
      ...
      options?: { restoreToComposerOnFailure?: boolean },
    ): Promise<boolean> => {
      const api = readNativeApi();
      if (!api || !activeThread || !activeProject || isSendBusy || isConnecting || sendInFlightRef.current) {
        return false;
      }
      const promptForSend = snapshot.prompt;
      const trimmed = snapshot.trimmedPrompt;
      const composerImagesSnapshot = snapshot.images;
      const composerTerminalContextsSnapshot = snapshot.terminalContexts;
      ...
      if (snapshot.expiredTerminalContextCount > 0) {
        const toastCopy = buildExpiredTerminalContextToastCopy(
          snapshot.expiredTerminalContextCount,
          "omitted",
        );
        toastManager.add({
          type: "warning",
      ...
      if (options?.restoreToComposerOnFailure) {
        promptRef.current = "";
      ...
      await (async () => {
        ...
        if (
          options?.restoreToComposerOnFailure &&
          !turnStartSucceeded &&
  ...
  async function submitComposer(action: "dispatch" | "queue") {
    if (!activeThread) {
      return;
    }
    ...
    const snapshot = buildComposerSubmissionSnapshot();
    if (!snapshot) {
      return;
    }
    if (showPlanFollowUpPrompt && activeProposedPlan) {
      if (action === "queue") {
        toastManager.add({
          type: "warning",
          title: "Plan follow-ups can't be queued yet",
          description: "Send this follow-up directly to refine or implement the current plan.",
        });
        return;
      }
    ...
    const standaloneSlashCommand =
      snapshot.images.length === 0 && snapshot.terminalContexts.length === 0
        ? parseStandaloneComposerSlashCommand(snapshot.trimmedPrompt)
        : null;
    ...
    if (action === "queue") {
      enqueueSubmission(snapshot, "append");
      return;
    }
    await dispatchSubmission(snapshot, { restoreToComposerOnFailure: true });
  }

  const onSend = async (e?: { preventDefault: () => void }) => {
    e?.preventDefault();
    await submitComposer("dispatch");
  };

  const drainQueuedSubmissionsIfPossible = useCallback(async () => {
    if (
    ...
    const nextQueuedPrompt = shiftComposerQueuedPrompt(activeThread.id);
    if (!nextQueuedPrompt) {
      queueDrainInFlightRef.current = false;
      return;
    }
    const didDispatch = await dispatchSubmission(
      {
        prompt: nextQueuedPrompt.prompt,
        trimmedPrompt: nextQueuedPrompt.prompt.trim(),
        terminalContexts: [],
    ...
    if (!didDispatch) {
      restoreComposerQueuedPromptToFront(activeThread.id, nextQueuedPrompt);
    }
    queueDrainInFlightRef.current = false;
  }, [
    activePendingApproval,
    ...
    restoreComposerQueuedPromptToFront,
    shiftComposerQueuedPrompt,
  ]);

  useEffect(() => {
    void drainQueuedSubmissionsIfPossible();
  }, [drainQueuedSubmissionsIfPossible]);
  ...
  const providerEffortButton = renderProviderEffortButton({
    provider: selectedProvider,
    threadId,
    model: selectedModel,
    models: selectedProviderModels,
  ...
  const onComposerCommandKey = (
    ...
    if (isOppositeFollowUpShortcut) {
      void submitComposer(effectiveFollowUpBehavior === "queue" ? "dispatch" : "queue");
      return true;
    }

    if (key === "Enter" && !event.shiftKey && !event.metaKey && !event.ctrlKey && !event.altKey) {
      void submitComposer(effectiveFollowUpBehavior === "queue" ? "queue" : "dispatch");
      return true;
    }
    return false;
  };
  const onSteerQueuedPrompt = useCallback(
    async (entryId: string) => {
      const entry = queuedPrompts.find((candidate) => candidate.id === entryId);
      if (!entry) {
        return;
      }

      removeComposerQueuedPrompt(threadId, entryId);
      const didDispatch = await dispatchSubmission(
        {
          prompt: entry.prompt,
          trimmedPrompt: entry.prompt.trim(),
          terminalContexts: [],
          expiredTerminalContextCount: 0,
          images: [],
        },
      ...
      if (!didDispatch) {
        restoreComposerQueuedPromptToFront(threadId, entry);
      }
  ...
  const onEditQueuedPrompt = useCallback(
    (entryId: string) => {
      ...
      const nextPrompt = existingPrompt.length > 0 ? `${promptRef.current}\n\n${entry.prompt}` : entry.prompt;
      promptRef.current = nextPrompt;
      setPrompt(nextPrompt);
      const nextCursor = collapseExpandedComposerCursor(nextPrompt, nextPrompt.length);
      setComposerCursor(nextCursor);
    ...
    [queuedPrompts, removeComposerQueuedPrompt, scheduleComposerFocus, setPrompt, threadId],
  );
  const onDeleteQueuedPrompt = useCallback(
    (entryId: string) => {
      removeComposerQueuedPrompt(threadId, entryId);
      scheduleComposerFocus();
    },
    [removeComposerQueuedPrompt, scheduleComposerFocus, threadId],
  );
  ...
  const onOpenTurnDiff = useCallback(
    ...
    [navigate, threadId, threadRoutePath],
  );
    ...
    <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-x-hidden bg-background">
      ...
      <div className="flex min-h-0 min-w-0 flex-1">
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col">
          ...
          {/* Input bar */}
          <div className={cn("px-3 pt-1 sm:px-5 sm:pt-1.5", isGitRepo ? "pb-1" : "pb-2.5 sm:pb-3")}>
            <form
              ref={composerFormRef}
              onSubmit={onSend}
                    ...
                    >
                      ...
                      >
                        ...
                        <ProviderModelPicker
                          ...
                          triggerSize="xs"
                          triggerClassName="rounded-full px-2.5 text-muted-foreground/80 hover:text-foreground"
                        ...
                        <Tooltip>
                          <TooltipTrigger
                            render={
                              <Button
                                variant="ghost"
                                className={cn(
                                  "shrink-0 rounded-full px-2 hover:text-foreground",
                                  interactionMode === "plan"
                          ...
                          <TooltipPopup side="top">
                            {interactionMode === "plan" ? "Plan mode active" : "Switch to plan mode"}
                          </TooltipPopup>
                        </Tooltip>
                      ...
                      >
                        ...
                        {activePendingProgress ? (
                          ...
                          <button
                            type="button"
                            className="flex h-7 w-7 cursor-pointer items-center justify-center rounded-full bg-rose-500/90 text-white transition-all duration-150 hover:bg-rose-500 hover:scale-105 sm:h-6 sm:w-6"
                          ...
                          showPlanFollowUpPrompt ? (
                            ...
                            <button
                              type="submit"
                              className="flex h-7 w-7 enabled:cursor-pointer items-center justify-center rounded-full bg-primary/90 text-primary-foreground transition-all duration-150 hover:bg-primary hover:scale-105 disabled:pointer-events-none disabled:opacity-30 disabled:hover:scale-100 sm:h-6 sm:w-6"
                              disabled={
      ...
      {(() => {
        if (uiMode === "embedded" || !terminalState.terminalOpen || !activeProject) {
          return null;
        }
        return (
