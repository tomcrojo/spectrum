...
let spectrumClientSettingsBridgeInitialized = false;

// ── Key sets for routing patches ─────────────────────────────────────

const SERVER_SETTINGS_KEYS = new Set<string>(Struct.keys(ServerSettings.fields));

function isFollowUpBehavior(value: unknown): value is ClientSettings["followUpBehavior"] {
  return value === "queue" || value === "steer";
}

function isAssistantStreaming(value: unknown): value is boolean {
  return typeof value === "boolean";
}

async function syncSpectrumServerSettings(patch: Partial<ServerSettings>) {
  const api = ensureNativeApi();
  const current = await api.server.getSettings();
  const hasChanges = Object.entries(patch).some(
    ([key, value]) => current[key as keyof ServerSettings] !== value,
  );
...
function syncSpectrumClientSettings(patch: Partial<ClientSettings>) {
  const current =
    getLocalStorageItem(CLIENT_SETTINGS_STORAGE_KEY, ClientSettingsSchema) ?? DEFAULT_CLIENT_SETTINGS;
  const hasChanges = Object.entries(patch).some(
    ([key, value]) => current[key as keyof ClientSettings] !== value,
  );
  ...
  const next = Schema.decodeSync(ClientSettingsSchema)({
    ...current,
    ...patch,
  });
  setLocalStorageItem(CLIENT_SETTINGS_STORAGE_KEY, next, ClientSettingsSchema);
  dispatchLocalStorageChange(CLIENT_SETTINGS_STORAGE_KEY);
}

function getSpectrumFollowUpBehaviorFromSearch(): ClientSettings["followUpBehavior"] | null {
...
function getSpectrumAssistantStreamingFromSearch(): boolean | null {
  if (typeof window === "undefined") {
    return null;
  }

  const raw = new URLSearchParams(window.location.search).get("spectrumAssistantStreaming");
  if (raw === "1" || raw === "true") {
    return true;
  }
...
function initializeSpectrumClientSettingsBridge() {
  if (typeof window === "undefined" || spectrumClientSettingsBridgeInitialized) {
    return;
  }

  spectrumClientSettingsBridgeInitialized = true;

  const initialFollowUpBehavior = getSpectrumFollowUpBehaviorFromSearch();
  if (initialFollowUpBehavior) {
    syncSpectrumClientSettings({ followUpBehavior: initialFollowUpBehavior });
  }

  const initialAssistantStreaming = getSpectrumAssistantStreamingFromSearch();
  if (initialAssistantStreaming !== null) {
    void syncSpectrumServerSettings({ enableAssistantStreaming: initialAssistantStreaming });
  }
  ...
  window.addEventListener("message", (event) => {
    ...
    const nextFollowUpBehavior = data.settings?.followUpBehavior;
    if (isFollowUpBehavior(nextFollowUpBehavior)) {
      syncSpectrumClientSettings({ followUpBehavior: nextFollowUpBehavior });
    }

    const nextAssistantStreaming = data.settings?.assistantStreaming;
    if (isAssistantStreaming(nextAssistantStreaming)) {
      void syncSpectrumServerSettings({
        enableAssistantStreaming: nextAssistantStreaming,
      });
    }
  });
}

initializeSpectrumClientSettingsBridge();
