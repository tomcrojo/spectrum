...
describe("composerDraftStore queued prompts", () => {
  const threadId = ThreadId.makeUnsafe("thread-queued-prompts");

  beforeEach(() => {
    resetComposerDraftStore();
  });

  it("appends queued prompts in FIFO order and can promote one to the front", () => {
    useComposerDraftStore.getState().enqueueQueuedPrompt(threadId, {
      id: "queued-1",
    ...
    useComposerDraftStore.getState().moveQueuedPromptToFront(threadId, "queued-2");

    expect(
      useComposerDraftStore.getState().draftsByThreadId[threadId]?.queuedPrompts.map((entry) => entry.id),
    ).toEqual(["queued-2", "queued-1"]);
  });

  it("shifts and restores queued prompts at the front", () => {
    useComposerDraftStore.getState().enqueueQueuedPrompt(threadId, {
      id: "queued-1",
      prompt: "first follow-up",
      createdAt: "2026-03-30T12:00:00.000Z",
    ...
    const shifted = useComposerDraftStore.getState().shiftQueuedPrompt(threadId);
    expect(shifted?.id).toBe("queued-1");
    expect(
      useComposerDraftStore.getState().draftsByThreadId[threadId]?.queuedPrompts.map((entry) => entry.id),
    ).toEqual(["queued-2"]);

    useComposerDraftStore.getState().restoreQueuedPromptToFront(threadId, shifted!);
  ...
  it("keeps queued prompts when a local draft thread is promoted", () => {
    const projectId = ProjectId.makeUnsafe("project-queued-prompts");
    ...
    expect(useComposerDraftStore.getState().projectDraftThreadIdByProjectId[projectId]).toBeUndefined();
    expect(
      useComposerDraftStore.getState().draftsByThreadId[threadId]?.queuedPrompts.map((entry) => entry.id),
    ).toEqual(["queued-1"]);
  });
