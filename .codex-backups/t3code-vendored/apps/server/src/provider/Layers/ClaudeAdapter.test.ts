...
function makeHarness(config?: {
  ...
  ensureSpectrumBrowserRuntimeEnv();
...
const DEFAULT_BROWSER_ENV_DIR = mkdtempSync(path.join(os.tmpdir(), "claude-browser-env-"));

function ensureSpectrumBrowserRuntimeEnv(): void {
  process.env.SPECTRUM_BROWSER ??= path.join(DEFAULT_BROWSER_ENV_DIR, "browser");
  process.env.SPECTRUM_BROWSER_CLI ??= path.join(DEFAULT_BROWSER_ENV_DIR, "browser-cli");
  process.env.T3CODE_STATE_DIR ??= DEFAULT_BROWSER_ENV_DIR;
}

afterAll(() => {
  rmSync(DEFAULT_BROWSER_ENV_DIR, { recursive: true, force: true });
});
...
describe("ClaudeAdapterLive", () => {
  ...
  it.effect("uses thread-pinned browser shims instead of injecting per-thread API tokens", () => {
    const harness = makeHarness();
    const baseDir = mkdtempSync(path.join(os.tmpdir(), "claude-browser-shim-"));
    const originalBrowser = process.env.SPECTRUM_BROWSER;
    const originalBrowserCli = process.env.SPECTRUM_BROWSER_CLI;
    const originalStateDir = process.env.T3CODE_STATE_DIR;
    const originalApiPort = process.env.SPECTRUM_API_PORT;
    const originalApiToken = process.env.SPECTRUM_API_TOKEN;
    const originalWorkspaceId = process.env.SPECTRUM_WORKSPACE_ID;
    const originalProjectId = process.env.SPECTRUM_PROJECT_ID;

    process.env.SPECTRUM_BROWSER = path.join(baseDir, "real-browser");
    process.env.SPECTRUM_BROWSER_CLI = path.join(baseDir, "real-browser-cli");
    process.env.T3CODE_STATE_DIR = baseDir;
    delete process.env.SPECTRUM_API_PORT;
    delete process.env.SPECTRUM_API_TOKEN;
    delete process.env.SPECTRUM_WORKSPACE_ID;
    ...
    return Effect.gen(function* () {
      ...
      try {
        ...
        const createInput = harness.getLastCreateQueryInput();
        const browserCliPath = createInput?.options.env?.SPECTRUM_BROWSER_CLI;
        const browserPath = createInput?.options.env?.SPECTRUM_BROWSER;
        assert.equal(createInput?.options.env?.SPECTRUM_API_TOKEN, undefined);
        assert.equal(createInput?.options.env?.SPECTRUM_API_PORT, undefined);
        assert.equal(createInput?.options.env?.SPECTRUM_BROWSER_THREAD_ID, THREAD_ID);
        assert.equal(browserCliPath === process.env.SPECTRUM_BROWSER_CLI, false);
        assert.equal(browserPath === process.env.SPECTRUM_BROWSER, false);
        assert.equal(typeof browserCliPath, "string");
        assert.equal(typeof browserPath, "string");
        if (!browserCliPath || !browserPath) {
          return;
        }

        const shimDir = path.dirname(browserCliPath);
        assert.equal(path.dirname(browserPath), shimDir);
        assert.equal(createInput?.options.env?.PATH?.startsWith(`${shimDir}${path.delimiter}`), true);

        yield* adapter.stopSession(THREAD_ID);
        assert.equal(existsSync(shimDir), false);
      } finally {
        if (originalBrowser === undefined) {
          delete process.env.SPECTRUM_BROWSER;
        } else {
          process.env.SPECTRUM_BROWSER = originalBrowser;
        }
        if (originalBrowserCli === undefined) {
          delete process.env.SPECTRUM_BROWSER_CLI;
        } else {
          process.env.SPECTRUM_BROWSER_CLI = originalBrowserCli;
        }
        if (originalStateDir === undefined) {
          delete process.env.T3CODE_STATE_DIR;
        } else {
          process.env.T3CODE_STATE_DIR = originalStateDir;
        }
        if (originalApiPort === undefined) {
          delete process.env.SPECTRUM_API_PORT;
        } else {
          process.env.SPECTRUM_API_PORT = originalApiPort;
        }
        if (originalApiToken === undefined) {
          delete process.env.SPECTRUM_API_TOKEN;
        } else {
          process.env.SPECTRUM_API_TOKEN = originalApiToken;
        }
        if (originalWorkspaceId === undefined) {
          delete process.env.SPECTRUM_WORKSPACE_ID;
        } else {
          process.env.SPECTRUM_WORKSPACE_ID = originalWorkspaceId;
        }
        if (originalProjectId === undefined) {
          delete process.env.SPECTRUM_PROJECT_ID;
        } else {
          process.env.SPECTRUM_PROJECT_ID = originalProjectId;
        }
